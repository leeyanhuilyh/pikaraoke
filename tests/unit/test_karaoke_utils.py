...
class TestTransposeCurrent:
    """Tests for transpose_current, which changes pitch live without restarting."""
    ...
    def test_transpose_updates_pitch_without_skipping(self, mock_karaoke_with_songs):
        """A successful live pitch change never touches the queue or skips."""
        k = mock_karaoke_with_songs
        self._start_playing(k)

        k.transpose_current(2)

        assert k.playback_controller.now_playing_transpose == 2
        assert k.playback_controller.skipped_reasons == []
        assert len(k.queue_manager.queue) == 0

    def test_transpose_notifies_on_failure(self, mock_karaoke_with_songs):
        """If the live zmq command fails, the pitch must not appear to have changed."""
        k = mock_karaoke_with_songs
        self._start_playing(k)
        k.playback_controller.set_pitch_should_succeed = False

        k.transpose_current(2)

        assert k.playback_controller.now_playing_transpose == 0
        assert k.now_playing_notification == "Failed to change pitch::is-danger"
    ...
    def test_transpose_with_nothing_playing_does_nothing(self, mock_karaoke_with_songs):
        ...
        assert mock_karaoke_with_songs.playback_controller.now_playing_transpose == 0
